...
import com.nttdocomo.android.tvterminalapp.webapiclient.jsonparser.ClipJsonParser;
...
public class ClipWebClient
        extends WebApiBasePlala implements WebApiBasePlala.WebApiBasePlalaCallback {

    /**
     * コールバック
     */
    public interface ClipJsonParserCallback {
        /**
    ...
    //コールバックのインスタンス
    private ClipJsonParserCallback mClipJsonParserCallback;
    ...
    public void onAnswer(ReturnCode returnCode) {
        //JSONをパースして、データを返す
        new ClipJsonParser(mClipJsonParserCallback).execute(returnCode.bodyData);
    }
     ...
     * @param clipJsonParserCallback callback
     * @return パラメータエラー等が発生した場合はfalse
     */
    public boolean getChannelApi(String type, String crid, String serviceId, String eventId,
                                 String titleId, String title, String r_value,
                                 String linearStartDate, String linearEndDate, boolean isNotify,
                                 ClipJsonParserCallback clipJsonParserCallback) {
        //パラメーターのチェック
        if (!checkFirstHalfParameter(type, crid, serviceId, eventId, titleId, isNotify)) {
            //パラメーターがおかしければ通信不能なので、falseで帰る
            return false;
        }

        //パラメーターのチェック
        if (!checkLatterHalfParameter(type, title, r_value, linearStartDate, linearEndDate,
                isNotify, clipJsonParserCallback)) {
            //パラメーターがおかしければ通信不能なので、falseで帰る
            return false;
        }

        //コールバックの設定
        mClipJsonParserCallback = clipJsonParserCallback;
     ...
     */
    private boolean checkFirstHalfParameter(String type, String crid, String serviceId, String eventId,
                                            String titleId, boolean isNotify) {
        //文字列がヌルならfalse
        if (type == null) {
            return false;
        }
    ...
    private boolean checkLatterHalfParameter(String type, String title, String r_value,
        ...
        if (clipJsonParserCallback == null) {
            //コールバックがヌルならばfalse
            return false;
        }
