 ...
 *  @param[in] time time value to set in the the du_scheduled_task data.
 *  @return  true if the function succeeds.
 *           false if the function fails.
 *  @remark This function is multithread safe.
 */
extern du_bool du_scheduler_modify_scheduled_task(du_scheduler* x, du_uint32 id, du_time time);
