...
/** @file dupnp_dvcdsc_array.h
 *  @brief The dupnp_dvcdsc_array interface provides various methods for manipulating arrays of
 *   dupnp_dvcdsc structures ( such as allocation, initialization, searching, concatenation ).
 *   Before using a dupnp_dvcdsc_array, use <b>dupnp_dvcdsc_array_init</b> to
 *   initialize dupnp_dvcdsc_array.
 *   The dupnp_dvcdsc_array supports arrays that can dynamically grow as necessary.
 *   Array indexes always start at position 0.
 *   Deleting the allocated memory of dupnp_dvcdsc_array, use <b>dupnp_dvcdsc_array_free</b>.
 *  @see dupnp_dvcdsc.h, dupnp_dvc.h
 */

#ifndef DUPNP_DVCDSC_ARRAY_H
#define DUPNP_DVCDSC_ARRAY_H
...
/**
 * Resizable array which can contains dupnp_dvcdsc values.
 * @see dupnp_dvcdsc.h, dupnp_dvc.h
 */
typedef du_array dupnp_dvcdsc_array;

/**
 * Initializes an array.
 * @param[out] x   pointer to the array.
 */
#define dupnp_dvcdsc_array_init(x) du_array_init((x), sizeof(dupnp_dvcdsc))
 ...
 */
#define dupnp_dvcdsc_array_allocate(x, len) du_array_allocate((x), (len))
 ...
 *   0 if x has failed, or not enough memory is available.
 */
#define dupnp_dvcdsc_array_allocate_align(x, len) du_array_allocate_align((x), (len))
 ...
 */
#define dupnp_dvcdsc_array_get(x) ((dupnp_dvcdsc*)du_array_get(x))

/**
 * Gets a pointer to the element stored in <em>x</em> by position.
 * @param[in] x pointer to the arrayl
 * @param[in] pos the position of element.
 * @return
 *   a pointer to the element.
 *   0 if x has failed, or if fewer than pos + 1 elements are used.
 */
#define dupnp_dvcdsc_array_get_pos(x, pos) du_array_get_pos((x), (pos))

/**
 * Finds the element which has the specified <em>udn</em>
 ...
 */
extern du_uint32 dupnp_dvcdsc_array_find_udn(const dupnp_dvcdsc_array* x, const du_uchar* udn);
 ...
 */
#define dupnp_dvcdsc_array_length(x) du_array_length(x)
 ...
 */
#define dupnp_dvcdsc_array_bytes(x) du_array_bytes(x)
 ...
 */
#define dupnp_dvcdsc_array_truncate_length(x, len) du_array_truncate_length((x), (len))
 ...
 *          false if the number of elements is not changed.
 */
extern du_bool dupnp_dvcdsc_array_truncate_length_object(dupnp_dvcdsc_array* x, du_uint32 len);
 ...
 * @return  true.
 */
#define dupnp_dvcdsc_array_truncate(x) du_array_truncate(x)
 ...
 * @return  true.
 */
extern du_bool dupnp_dvcdsc_array_truncate_object(dupnp_dvcdsc_array* x);

/**
 * Frees the region used by <em>x</em>l
 * @param[in] x pointer to the array.
 */
#define dupnp_dvcdsc_array_free(x) du_array_free(x)

/**
 * Frees the region used by <em>x</em> and frees the all element objects.
 * @param[in] x pointer to the array.
 */
extern void dupnp_dvcdsc_array_free_object(dupnp_dvcdsc_array* x);
 ...
 */
#define dupnp_dvcdsc_array_failed(x) du_array_failed(x)
 ...
 */
#define dupnp_dvcdsc_array_equal(x, y) du_array_equal((x), (y))

/**
 * Appends all elements in the source array to the destination array allocating more space if necessary.
 ...
 */
#define dupnp_dvcdsc_array_cat(to, from) du_array_cat((to), (from))
 ...
 */
#define dupnp_dvcdsc_array_catn(to, from, len) du_array_catn((to), (du_uint8*)(from), (len))
 ...
 */
#define dupnp_dvcdsc_array_cato(to, object_ptr) du_array_cato((to), (du_uint8*)(object_ptr))
 ...
 */
#define dupnp_dvcdsc_array_remove(x, pos) du_array_remove((x), (pos))
 ...
 */
extern du_bool dupnp_dvcdsc_array_remove_object(dupnp_dvcdsc_array* x, du_uint32 pos);
