...
struct du_netif {
    ...
    /**
     *   Whether or not this interface is loopback.
     */
    du_bool loopback;
};
...
/**
 *  Retrieves all IP network interfaces with the filter specified.
 *
 *  This function can be used for retrieving both IPv6 and IPv4 interfaces.
 *  In this API, callback function which is set by du_netif_set_netif_list_filter() isn't called.
 *
 *  @param[in] filter a callback function implements <em>du_netif_list_filter</em> interface.
 *  @param[in] arg a pointer to user data which is passed to the <em>filter</em>.
 ...
 *  the <b>du_netif_array_init</b> function.
 */
extern du_bool du_netif_get_list3(du_netif_list_filter filter, void* arg, du_netif_array* array);
