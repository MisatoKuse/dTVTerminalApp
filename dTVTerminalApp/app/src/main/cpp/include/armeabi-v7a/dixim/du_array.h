 ...
 *  @pre @p x must be initialized by du_array_init().
 */
#define du_array_get(x) ((void*)(x)->p)
...
#ifdef DU_INLINE_MODE
#if DU_INLINE_MODE == 99
inline void* du_array_allocate_impl(du_array* x, du_uint32 member_size, du_uint32 len, du_bool align) {
#elif DU_INLINE_MODE == 89
extern inline void* du_array_allocate_impl(du_array* x, du_uint32 member_size, du_uint32 len, du_bool align) {
#endif
    du_uint32 wanted;
    du_uint32 recommended;
    du_uint64 mult;
    void* p;

    if (x->failed) return 0;

    mult = (du_uint64)member_size * (du_uint64)len;
    if (0xffffffff < mult) return 0;
    wanted = (du_uint32)mult & DU_UINT32_CONST(0xffffffff);

    if (x->allocated < wanted) {
        recommended = x->allocated + (x->allocated >> 3);
        wanted = (recommended < wanted) ? wanted : recommended;
        if (align) {
            // round up
            if (member_size < 8) {
                wanted = (wanted + 0x7f) & DU_UINT32_CONST(0xffffff80);
                if (wanted < 0x80) return 0;
            } else {
                wanted = (wanted + 0xfff) & DU_UINT32_CONST(0xfffff000);
                if (wanted < 0x1000) return 0;
            }
        }

        p = du_alloc_re(x->p, x->used, wanted);
        if (!p) return p;
        x->p = (du_uint8*)p;
        x->allocated = wanted;
    }
...
#if DU_INLINE_MODE == 99
inline du_bool du_array_catn(du_array* to, const du_uint8* from, du_uint32 len) {
#elif DU_INLINE_MODE == 89
extern inline du_bool du_array_catn(du_array* to, const du_uint8* from, du_uint32 len) {
#endif
    du_uint32 wanted;
    du_uint64 mult;

    if (!len) return 1;
    if (to->failed) return 0;

    mult = (du_uint64)to->member_size * (du_uint64)len;
    if (0xffffffff < mult) return 0;
    wanted = (du_uint32)mult & DU_UINT32_CONST(0xffffffff);

    if (to->used + wanted < to->used) goto error;
    if (!du_array_allocate_impl(to, sizeof(du_uint8), to->used + wanted, 1)) goto error;
    du_byte_copy(to->p + to->used, wanted, from);
...
#else
extern void* du_array_allocate_impl(du_array* x, du_uint32 member_size, du_uint32 len, du_bool align);
