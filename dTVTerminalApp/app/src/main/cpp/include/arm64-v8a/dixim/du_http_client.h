...
#define DU_HTTP_CLIENT_SCOPE_ID_AUTO (0)
...
/**
 * Sets a scope to be used to connect to the remote peer on a link local network.
 *
 * @param[in] hc pointer to the du_http_client structure.
 * @param[in] scope_id Scope ID of IPv6 link local network.(See remarks)
 * @return true if the function succeeded, false otherwise.
 * @remarks
 *  This value is used only when connecting to the remote peer on
 *  IPv6 link local network.
 * @remarks
 *  `scope_id` must be a value of `index` of du_netif
 *  which is fetched by calling du_netif_get_list3().
 * @remarks
 *  The default is DU_HTTP_CLIENT_SCOPE_ID_AUTO and
 *  it tries to use the first connectable
 *  scope found on the system.
 */
extern du_bool du_http_client_set_scope_id(du_http_client* hc, du_uint32 scope_id);

/**
 * Sets IP version to be used.
 *
 * @param[in] hc pointer to the du_http_client structure.
 * @param[in] ip_version IP version to be used.
 * @return true if the function succeeded, false otherwise.
 * @remarks
 *  Default value is DU_IP_FAMILY_UNKNOWN and tries to use IP version available on the system.
 * @remarks
 *  Request will fail if the platform doesn't have any interfaces
 *  support IP version specified.
 */
extern du_bool du_http_client_set_ip_version(du_http_client* hc, du_ip_family ip_version);
 ...
 *  @param[in] hc pointer to the du_http_client structure.
 *  @param[in] sock socket handle to set.
 */
extern void du_http_client_set_socket(du_http_client* hc, du_socket sock);
