 ...
 */
typedef void (*du_scheduled_task_handler)(du_time now, du_time* t, void* arg);
 ...
 *  @param[in] task pointer to the du_scheduled_task structure.
 *  @param[in] t time value to invoke a scheduled_task_handler() function.
 *  @remark task is a pointer to a du_scheduled_task structure
 *  initialized by the <b>du_scheduled_task_init</b> function.
 */
extern void du_scheduled_task_set_time(du_scheduled_task* task, du_time t);
