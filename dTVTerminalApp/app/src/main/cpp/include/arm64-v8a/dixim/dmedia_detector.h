...
/**
 *  An interface definition of a handler.
 *  <b>dmedia_detector_event_handler2</b> function is an application-defined
 *    callback function when the media detector is detected filesystem changes.
 *  @param[in] path root path which changed own path or any the descendant pathes.
 *  @param[in] sub_root_path_array sub root path which changed own path or any the descendant pathes.
 *             sub root path's ancestor pathes aren't changed.
 *  @param[in] arg a parameter for the handler function.
 *  @remark sub_root_path_array is 0 except linux platform.
 */
typedef void (*dmedia_detector_event_handler2)(const du_uchar* path, const du_str_array* sub_root_path_array, void* arg);

typedef void (*dmedia_detector_event_handler3)(const du_uchar* path, const du_str_array* sub_root_path_array, const du_str_array* path_array, void* arg);
...
/**
 *  Registers a <b>dmedia_detector_event_handler2</b> function in dmedia_detector.
 *  The <b>dmedia_detector_event_handler2</b> function is an application-defined
 *    callback function invoked when the media detector is detected filesystem changes.
 *  @param[in] x pointer to the dmedia_detector structure.
 *  @param[in] handler the <b>dmedia_detector_event_handler2</b> function to store in upnp.
 *  @param[in] arg pointer to the user defined data area.
 */
extern void dmedia_detector_set_event_handler2(dmedia_detector* x, dmedia_detector_event_handler2 handler, void* arg);

extern void dmedia_detector_set_event_handler3(dmedia_detector* x, dmedia_detector_event_handler3 handler, void* arg, du_uint32 max_path_array_len);
