...
du_bool dmp_conf_get_download_dir_path(const du_uchar* conf_path, du_uchar_array* download_dir_path, du_uchar_array* tmp_ua) {
    ...
    if (!get_conf_path(conf_path, DU_UCHAR_CONST("download_dir"), tmp_ua)) return 0;
    if (!du_uchar_array_cat0(tmp_ua)) return 0;
    if (!du_file_get_first_line(du_uchar_array_get(tmp_ua), str, sizeof str)) return 0;
    if (!du_uchar_array_cats(download_dir_path, str)) return 0;
    return 1;
}
...
du_bool dmp_conf_get_dirag_path(const du_uchar* conf_path, du_uchar_array* dirag_path) {
    if (!get_conf_path(conf_path, DU_UCHAR_CONST("drag_configuration.xml"), dirag_path)) return 0;
    return 1;
}
