...
static du_bool isTargetDevice(xmlNodePtr node, const du_uchar* udn, const du_uchar* device_type) {
    ...
    // UDN(UUID) を取得する
    nptr = du_libxml_get_element_by_name(node->children, URN_DEVICE, DU_UCHAR_CONST(DMS_UUID_ELEMENT));
    if (!nptr){
        goto error;
    }
    // UDN(UUID)を比較
     ...
     * デバイスタイプ要素
     */
    device_type_ = du_libxml_get_content_by_name_s(node->children, URN_DEVICE, DU_UCHAR_CONST(DMS_DEVICE_TYPE), &ua);
    if (!device_type_){
        goto error;
    }
    // デバイスタイプを比較
 ...
 * @return
 */
static xmlNodePtr findUdnXmlDoc(xmlDocPtr doc, xmlNodePtr children, const du_uchar* udn, const du_uchar* device_type) {
    xmlNodePtr n;
    xmlNodePtr n2;
    ...
    for (n = children; n; n = n->next) {
        ...
        n2 = du_libxml_get_element_by_name(n->children, URN_DEVICE, DU_UCHAR_CONST(DMS_DEVICE_LIST_ELEMENT));
        if (!n2){
            continue;
        }
        // 子要素の検索
...
static xmlNodePtr findServiceXmlDoc(xmlNodePtr children, const du_uchar* service_type) {
    ...
    for (n = children; n; n = n->next) {
        ...
        n2 = du_libxml_get_element_by_name(n->children, URN_DEVICE, DU_UCHAR_CONST(DMS_SERVICE_TYPE));
        if (!n2){
            continue;
        }
...
static du_bool setDmsInfoXmlDoc(dms_info* info, xmlNodePtr node, const du_uchar* location) {
    ...
    // UUIDを設定
    if (!du_libxml_get_content_by_name(node->children, URN_DEVICE, DU_UCHAR_CONST(DMS_UUID_ELEMENT), &ua)){
        goto error;
    }
    if (!du_uchar_array_cat0(&ua)){
        goto error;
    ...
    // サービスリストを設定
    if (!du_libxml_get_content_by_name(node->children, URN_DEVICE, DU_UCHAR_CONST(DMS_FRIENDLY_NAME_ELEMENT), &ua)){
        goto error;
    }
    if (!du_uchar_array_cat0(&ua)){
        goto error;
    ...
    // サービスリストを設定
    nptr = du_libxml_get_element_by_name(node->children, URN_DEVICE, DU_UCHAR_CONST(DMS_SERVICE_LIST));
    if (!nptr){
        goto error;
    }
    ...
    // コントロールURLを設定
    if (!du_libxml_get_content_by_name(nptr->children, URN_DEVICE, DU_UCHAR_CONST(DMS_CONTROL_URL), &ua)){
        goto error;
    }
    if (!du_uchar_array_cat0(&ua)){
        goto error;
    ...
    // イベントサブURLを設定
    if (!du_libxml_get_content_by_name(nptr->children, URN_DEVICE, DU_UCHAR_CONST(DMS_EVENTSUB_URL), &ua)){
        goto error;
    }
    if (!du_uchar_array_cat0(&ua)){
        goto error;
...
static du_bool findUrlBaseXmlDoc(xmlNodePtr node, du_uchar_array* ua) {
    if (!du_libxml_get_content_by_name(node->children, URN_DEVICE, DU_UCHAR_CONST(DMS_URLBASE_ELEMENT), ua)){
        return 0;
    }
    return 1;
}
 ...
 * @return
 */
dms_info* createDmsInfoXmlDoc(const du_uchar* xml, du_uint32 xml_len, const du_uchar* udn, const du_uchar* device_type, const du_uchar* location) {
    xmlDocPtr doc = 0;
    xmlNodePtr node;
    dms_info* info = 0;
    ...
    // デバイスディスクリプションをパースする
    doc = xmlReadMemory((const char*)xml, (int)xml_len, DMS_DEVICE_DESCRIPTION, 0, XML_PARSE_NOERROR | XML_PARSE_NOWARNING | XML_PARSE_NOBLANKS | XML_PARSE_NONET);
    if (!doc){
        goto error;
    }
